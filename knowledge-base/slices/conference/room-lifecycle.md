...
房间生命周期负责一场 Conference 标准会议从“被创建”到“被结束”的主链路，覆盖即时会议发起、加入已有房间、在无法确认房间是否已存在时直接创建并加入、主动离房、房主结束会议以及被动退房后的状态收口。它是会议产品的主干流程，服务端预创建房间或代解散房间时，前端仍需通过同一套生命周期能力完成真实入会和退场。
