...
它消费房间、成员和共享状态，但**不负责**创建或加入房间，也不负责打开摄像头、麦克风或管理成员权限。标准会议里，它通常作为 `room-lifecycle`、`participant-list`、`screen-share`、`device-control` 之上的“画面呈现层”。
...
| 方式 | 适用场景 | 说明 |
|------|----------|------|
| 房间级布局容器 | 标准会议、常规会中页面 | 更适合承接房间内所有参与者流的排序、播放、布局排版和缩放处理，并统一消费共享状态和焦点状态 |
...
3. **标准会议优先使用房间级布局容器，自定义布局再下沉到单路播放器** —— 标准会议更适合复用内置排序、播放、缩放和布局编排；只有在布局自由度明显超出标准会议能力时，再采用逐路播放器自行拼装。
4. **为屏幕共享保留明确优先级和回退规则** —— 标准会议通常在有共享时切到主画面布局，无共享或房间内仅一人时回到宫格，避免主画面悬空。
