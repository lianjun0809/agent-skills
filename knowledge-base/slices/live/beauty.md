...
| 滑块与效果不同步 | 未订阅 `state` | 建立 Combine 订阅，从 `state` 读取参数更新 UI |
| `reset()` 后滑块未归零 | UI 未监听状态变化 | 订阅 `$state` 在 `reset()` 后自动刷新 UI |
...
reset() 后 UI 未更新
└── 检查是否订阅了 BaseBeautyStore.shared.$state
    └── 在 sink 中更新所有滑块的 value
```
